...
function ViewUser() {
  ...
  const getAllUsers = async () => {
    //const response = await axios.get("http://192.168.0.5:8081/getUser");
    const user = { roleId: 3 };
    const response = await axios.post(
      "http://192.168.0.5:8081/getUserByRoleId",
      user
    );
  ...
  const myfun = async (userEmail) => {
    ...
    const res = await axios.post(
      "http://192.168.0.5:8081/getUserByEmail",
      user
    );
    ...
    if (res.data.status === "Active") {
      ...
      const user2 = await axios.post(
        "http://192.168.0.5:8081/updateUser",
        user1
      );
    } else {
      const user1 = {
